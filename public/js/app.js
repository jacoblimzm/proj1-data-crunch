...
let createTiles = () => {
  for (let i = 0; i < num ** 2; i++) {
    ...
    // generate randomNumber to give a random colour
    let randomColor = Math.floor(Math.random() * colors.length);
    console.log(randomColor);
    $divTile.css("background-color", colors[randomColor])
...
let clickSwap = (e) => {
  ...
  if (colorSwap1 !== undefined && idSwap1 !== idSwap2) {
    ...
    // if there is no existing id stored in idSwap1 and color stored in colorSwap1, do so upon click.
